...
void SceneLoader::readScene(const std::string &fileName, SceneData &sceneData)
{
    ...
    try
    {
        ...
        sceneData.m_camPosition = Vector3r(5.0f,10.0f,30.0f);
        readVector(m_json,"cameraPosition",sceneData.m_camPosition);
        sceneData.m_camLookat = Vector3r(5.0f,0.0f,0.0f);
        readVector(m_json,"cameraLookat",sceneData.m_camLookat);

        // read general
        sceneData.m_timeStepSize = 0.005f;
        sceneData.m_gravity = Vector3r(0.0f,-9.81f,0.0f);
...
void Utils::SceneLoader::readTriangleModels(const nlohmann::json &j, const std::string &key, const std::string &basePath, SceneData &sceneData)
{
	const nlohmann::json& child = j[key];
...
void Utils::SceneLoader::readTetModels(const nlohmann::json &j, const std::string &key, const std::string &basePath, SceneData &sceneData)
{
	const nlohmann::json& child = j[key];
...
void Utils::SceneLoader::readBallJoints(const nlohmann::json &j, const std::string &key, SceneData &sceneData)
{
	const nlohmann::json &child = j[key];
...
void Utils::SceneLoader::readBallOnLineJoints(const nlohmann::json &j, const std::string &key, SceneData &sceneData)
{
	const nlohmann::json &child = j[key];
...
/*
这是从json文件中提取出来配置参数，然后设置函数参数指针中
*/
